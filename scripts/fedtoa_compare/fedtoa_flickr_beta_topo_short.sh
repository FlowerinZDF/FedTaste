...
beta_topo_values=(1e-3 1e-2 1e-1)

for beta_topo in "${beta_topo_values[@]}"; do
  log_file="logs/fedtoa_compare/fedtoa_flickr_short_beta_topo_${beta_topo}_$(date +%Y%m%d_%H%M%S).log"
  mkdir -p "$(dirname "$log_file")"

  echo "[fedtoa-calib] dataset=Flickr30k rounds=${rounds} local_epochs=${local_epochs} beta_topo=${beta_topo}"
  echo "[fedtoa-calib] configured clients: teachers=${mc}, students=${student_clients} (img=${ic}, txt=${tc})"
  echo "[fedtoa-calib] writing run log to ${log_file}"
